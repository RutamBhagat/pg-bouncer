...
function getErrorMessage(code: string | undefined, originalMessage: string): string {
  if (!code) {
...
function getSimplifiedMessage(originalMessage: string): string {
  if (originalMessage.includes('Connection terminated')) return 'Database connection was terminated unexpectedly';
  if (originalMessage.includes('ECONNREFUSED')) return 'Connection refused - database server may be down';
  if (originalMessage.includes('ENOTFOUND')) return 'Database host not found';
  if (originalMessage.includes('timeout') || originalMessage.includes('ETIMEDOUT')) return 'Database connection timed out';
  if (originalMessage.includes('ECONNRESET')) return 'Database connection was reset';
